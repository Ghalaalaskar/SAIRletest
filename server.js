...
app.delete('/api/deleteAuthUser/', async (req, res) => {
  const { email } = req.body;
  console.log(email, req.body);
  try {
    const user = await admin.auth().getUserByEmail(email);
    console.log('userrrrrrr ', user.uid);
    // Delete the user by UID
    const uid = user.uid;
    await admin.auth().deleteUser(user.uid);
    res
      .status(200)
      .send({ message: `Successfully deleted user with UID: ${uid}` });
  } catch (error) {
    ...
    if (pos && unit.lmsg) {
      ...
      if (maxSpeed !== 0) {
        ...
        if (driverSpeed > maxSpeed) {
          const driverQuerySnapshot = await db
            .collection('Driver')
          ...
          if (!driverQuerySnapshot.empty) {
            ...
            if (price !== 0) {
              const motorcycleSnapshot = await db
              ...
              if (!motorcycleSnapshot.empty) {
                ...
                const location = await fetchLocationWithGoogleMaps(
                  pos.y,
                  pos.x
                );
                ...
                if (!querySnapshot.empty) {
                  ...
                  if (distance > 30) {
                    ...
                    if (
                      ...
                      newViolationTime - lastViolation.data().time >
                      tenMinutesInSeconds
                    ) {
...
const calculatePrice = (driverSpeed, maxSpeed) => {
  let price = 0;
  if (maxSpeed <= 120) {
    if (driverSpeed <= maxSpeed + 10) price = 0;
    ...
    if (driverSpeed <= maxSpeed + 5) price = 0;
...
const processUnits2 = async (units, sessionId) => {
  for (const unit of units) {
    ...
    if (pos) {
      ...
      if (driverQuerySnapshot.empty) {
        console.log(
          `No driver found for GPS number: ${GPSserialnumber}. Skipping.`
        );
      } else {
        const driverid = driverQuerySnapshot.docs[0].data().DriverID;
        ...
        const motorcycleQuerySnapshot = await db
          .collection('Motorcycle')
          .where('GPSnumber', '==', GPSserialnumber)
          .get();
            ...
            if (recentSpeeds[GPSserialnumber].length < 2) {
              ...
              for (let i = 1; i < recentSpeeds[GPSserialnumber].length; i++) {
                ...
                if (deceleration <= -7) {
                  ...
                  const crashQuerySnapshot = await db
                    .collection('Crash')
                    .where('GPSnumber', '==', GPSserialnumber)
                    .where('driverID', '==', driverid)
                    .where('time', '>=', starttime)
                  ...
                  if (!crashQuerySnapshot.empty) {
                    console.log(
                      'Crash already recorded in the last 5 minutes.'
                    );
                  } else {
                    const location = await fetchLocationWithGoogleMaps(
                      pos.y,
        ...
        if (driverSpeed + 10 == maxSpeed) {
          ...
          if (!driverQuerySnapshot.empty) {
            ...
            if (!motorcycleSnapshot.empty) {
              ...
              // } else {
              await storePotentialViolation(
                PotentialViolationID,
                driverid,
                GPSserialnumber,
                location,
                position,
...
const storePotentialViolation = async (
  PotentialViolationID,
  driverid,
  GPSnumber,
  location,
