...
import { doc, getDoc, setDoc, query, collection, where, onSnapshot , updateDoc, getDocs} from 'firebase/firestore';
...
const EditMotorcycle = () => {
  ...
  const fetchAvailableDrivers = async () => {
    try {
      const driverQuery = query(collection(db, 'Driver'), where('available', '==', true));
  ...
  const handleUpdateMotorcycle = async (values) => {
    try {
      ...
      // Step 1: Update the motorcycle document
      const motorcycleDocRef = doc(db, 'Motorcycle', motorcycleId);
      await updateDoc(motorcycleDocRef, updatedData);
  
      // Step 2: Update the driver's availability and GPSnumber
      if (values.DriverID && values.DriverID !== 'None') {
        ...
        // If 'None' is selected, we need to handle the previous driver
        const prevDriverId = motorcycleData.DriverID;
        if (prevDriverId) {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    const { Model, GPSnumber, Type, Brand, LicensePlate, DriverID } = motorcycleData;
