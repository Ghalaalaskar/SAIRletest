...
const NotificationList = () => {
  ...
  const handleSelect = (value) => {
    ...
    const newType = newSelection.filter(
      (val) => val === "Reckless Violations" || val === "Normal Violations"
    );
    const newStatus = newSelection.filter(
      (val) => val === "Active" || val === "Revoked"
    );
    setFilters({ type: newType, status: newStatus });
  };

  const handleViewDetails = (record) => {
    // Update viewed violations in local storage
  ...
  const filteredViolations = violations
    .filter((violation) => {
      ...
      const formattedSearchDate = searchDate
        ? formatDate(new Date(searchDate).getTime() / 1000)
        : "";

      const matchesSearchQuery =
        driverName.toLowerCase().includes(searchQuery.toLowerCase()) ||
        licensePlate.toLowerCase().includes(searchQuery.toLowerCase());
      const matchesSearchDate = formattedSearchDate
        ? violationDate === formattedSearchDate
        : true;

      const matchesTypeFilter =
        filters.type.length === 0 ||
        (filters.type.includes("Reckless Violations") &&
          violation.isReckless) ||
  ...
  return (
    <>
      ...
      <div className="breadcrumb">
        ...
        <span> / </span>
        <a onClick={() => navigate("/notificationList")}>Notification List</a>
      </div>
      <main>
